...
class PlantDetailScreen extends StatelessWidget {
  ...
  void _showDeleteCareEventConfirmationDialog(BuildContext context, CareEvent careEvent) {
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return AlertDialog(
        title: const Text("Delete Care Event"),
        content: const Text("Are you sure you want to delete this care event?"),
        actions: [
          TextButton(
...
void _showDeleteReminderConfirmationDialog(BuildContext context, Reminder reminder) {
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return AlertDialog(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) =>
                            AddCareEventScreen(plant: plant)));
              },
            ...
            if (plant.careEvents.isEmpty)
              ...
              return Row(
                children: [
                  Expanded(
                    child: Text(
                      '${_formatDate(event.date)} - ${event.type}: ${event.notes}',
                    ),
                  ),
                  IconButton(
            ...
            const SizedBox(height: 8),
            if (plant.reminders.isEmpty) const Text("No reminders added yet.")
              else ...plant.reminders.map((Reminder reminder) => Text('${reminder.task} - ${reminder.frequency} - ${reminder.nextDue}')),
          ],
        ),
      ),
    );
